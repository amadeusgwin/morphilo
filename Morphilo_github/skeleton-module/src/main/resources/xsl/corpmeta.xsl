...
<!--  <xsl:key name="derivate" match="response/response[@subresult='derivate']/result/doc" use="str[@name='returnId']" /> -->
  ...
  <xsl:template match="/mycoreobject[contains(@ID,'_corpmeta_')]">
    ...
    <head>
      ...
      <h1 style="text-indent: 4em;">
        <xsl:value-of select="i18n:translate('editor.label.corpus')" />
      </h1>

      <xsl:call-template name="objectAction2">
        <xsl:with-param name="id" select="@ID" />
        <xsl:with-param name="deriv" select="structure/derobjects/derobject/@xlink:href" /> <!-- structure/derobjects/derobject/ -->
      </xsl:call-template>
  ...
  <xsl:template name="Filenumber">
    ...
    <xsl:param name="nof" select="concat('untagged-', $derivateID)" />
    <!-- document((concat('ifs:/',$deriv))//children/child[type='file']/@name -->
  </xsl:template>

  <xsl:template name="objectAction2">
    ...
    <xsl:param name="deriv" select="''" />

    <xsl:if test="$accessedit or $accessdelete">

      <div class="dropdown pull-right" style="margin-right:10px">
        <button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true">
          <span class="glyphicon glyphicon-cog" aria-hidden="true"></span> Korpusprojekt
          <span class="caret"></span>
        </button>
        <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
          <li role="presentation">
            <a role="menuitem" tabindex="-1" href="{$WebApplicationBaseURL}content/publish/corpmeta.xed?id={$id}">
          ...
          <!-- Loeschen nicht moeglich, da Verweis in morphilo-Objekten vorgenommen wird -->
          <!--
